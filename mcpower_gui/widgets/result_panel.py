...
class ResultPanel(QWidget):
    ...
    def __init__(
        ...
        self._script_edit.setStyleSheet(
            "font-family: monospace; font-size: 11px; color: #333; background: #f5f5f5;"
        )
        ...
        for sname in SCENARIO_ORDER:
            ...
            if has_correction:
                self._add_results_table(
                    tab_layout, sresults, mode, target_power, max_n
                )
    ...
    def _save_csv(self):
        ...
        if not path:
            return
        try:
            lines = self._results_table.to_csv_lines()

            # Add replication script
            lines.append("")
            lines.append("Replication Script")
            for script_line in self._script.splitlines():
