...
Scenarios test your power analysis under progressively more pessimistic assumptions about real-world data. Each scenario has parameters for general perturbations, residual perturbations (all model types), and mixed effects perturbations (when using clustered data):
...
These parameters control non-normal residual (error) distributions. They apply to **all model types** (OLS and mixed models), not just mixed models.

### Residual dists (default: Heavy tailed + Skewed checked)

The pool of non-normal distributions available for residual perturbation. When the residual coin flip activates in a given simulation, one distribution is randomly selected from this pool:
- **Heavy tailed** — t-distribution with `residual_df` degrees of freedom. Produces occasional extreme errors.
- **Skewed** — shifted chi-squared distribution. Produces asymmetric errors.

Both are checked by default. If only one is checked, that distribution is always used when activated. The actual generation happens in C++ for exact reproducibility.

### Residual change prob (0–1, default: Optimistic=0, Realistic=0.3, Doomer=0.8)

Probability that residuals use a non-normal distribution (randomly picked from checked residual dists) in a given simulation. A value of 0.3 means 30% of simulations have non-normal residuals.

### Residual df (2–50, default: Optimistic/Realistic=10, Doomer=5)

Degrees of freedom for the heavy-tailed or skewed residual distribution. Lower values = heavier tails. Only consumed when the residual coin flip activates.

## Mixed Effects Perturbations

These parameters are only active when the formula contains random effects (e.g. `(1|school)`). They control how the random-effect structure is perturbed across simulations.
