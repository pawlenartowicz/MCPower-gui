...
- **Test formula** — An optional alternative formula for significance testing. Leave empty to use the model formula. Useful when you want to generate data with one model but test significance with a different (e.g., simpler) model. When a test formula is entered, the available target tests are automatically filtered to show only effects present in the test formula.
